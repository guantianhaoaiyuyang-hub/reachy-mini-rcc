...
from robot.motion_controller_v3 import ReachyMotionControllerV3
from robot.motion_manager_v3 import MotionManagerV3
...
async def receive_play_and_move(
    ...
    while True:
        ...
        if event == Event.TTS_RESPONSE:
            ...
            answer_audio_bytes += len(packet.payload)
        ...
        if event == Event.ASR_ENDED:
            user_is_speaking = False
            await motion.change(MotionState.THINKING)
            print("用户说话结束，机器人进入思考姿态")
            continue
        ...
        if event == Event.TTS_ENDED:
            ...
            elapsed = (
                time.monotonic() - first_audio_time
                if first_audio_time is not None
                else 0.0
            )

            # 服务端发送结束后，机器人扬声器可能仍有音频缓冲。
            # 动作继续到估算的本地播放结束时间。
            remaining = max(
                0.20,
                audio_duration - elapsed + 0.35,
            )
            remaining = min(remaining, 30.0)

            print(
                "[V3.2] 服务端音频发送结束；"
                f"累计音频约 {audio_duration:.2f} 秒，"
                f"继续动作约 {remaining:.2f} 秒"
            )
...
async def run() -> None:
    ...
    with ReachyMini(
        ...
        controller = ReachyMotionControllerV3(mini)
        motion = MotionManagerV3(controller)
